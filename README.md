...
- **Appearance & About** – Settings let you choose system/light/dark, edit expense categories through a dedicated sheet, tweak the IRS mileage rate, erase data, and review an About/Licenses section that cites ZIPFoundation’s MIT license. Theme helpers in `FlexErrnTheme.swift` keep every card and tile consistent.
...
3. **Theming** – `FlexErrnTheme` defines gradients, hero gradients, and shadow colors used across cards, nav bars, and hero tiles. The `.flexErrnCardStyle()` modifier keeps card trains ready for any section, and new hero tiles queue under that design.
...
2. Select the `FlexErrn` scheme and run on a device (preferred for VisionKit/receipt scanning). The simulator uses the fallback camera picker.
...
- **Backing up** – Navigate to the Data tab and tap “Backup FlexErrn Data.” This writes `FlexErrnBackup.json` plus any referenced receipt JPEGs into a ZIP archive using ZIPFoundation’s `Archive`.
...
- `FlexErrnTests.swift` / `FlexErrnUITests.swift` exist but are currently placeholders; manual validation is strongly recommended.  
- Run `xcodebuild -scheme FlexErrn -destination 'platform=iOS Simulator,name=iPhone 15' build` to ensure the project compiles after changes.  
